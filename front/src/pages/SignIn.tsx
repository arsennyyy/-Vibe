...
const SignIn = () => {
  ...
  return (
    <Layout>
      <div className="relative flex flex-col items-center justify-center min-h-[calc(100vh-100px)] px-4 py-16 overflow-hidden bg-[#0a0a0a]">
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <h2 className="text-3xl font-bold mb-2 font-display text-white tracking-tight">
            {step === "form" ? "Добро пожаловать" : "Почти готово"}
          </h2>
          <p className="text-white/50 text-sm font-medium">
            {step === "form" ? "Войдите в свой аккаунт" : "Осталось подтвердить код из письма"}
          </p>
        </div>
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="w-full max-w-[420px] bg-[#161616] border border-white/10 rounded-3xl p-8 shadow-xl relative z-10"
        >
          {error && (
          ...
          {step === "otp" ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-2">
                <Label htmlFor="email" className="text-[10px] uppercase tracking-[0.15em] text-white/45 font-bold ml-1">
                  Электронная почта
                </Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="password" className="text-[10px] uppercase tracking-[0.15em] text-white/45 font-bold ml-1">
                  Пароль
                </Label>
                <div className="relative">
                  <Lock className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-white/45" />
                  <Input
                    id="password"
            ...
            <>
              <div className="relative my-8">
                ...
                <div className="relative flex justify-center text-[10px] uppercase tracking-[0.2em]">
                  <span className="bg-[#161616] px-4 text-white/45 font-bold">или</span>
                </div>
              </div>
              <GoogleSignInButton text="signin_with" onSuccess={(c) => void googleLogin(c)} />
            </>
          )}
        ...
        <CaptchaModal
          ...
          description="Перед отправкой кода на почту пройдите короткую проверку."
        />
      </div>
    </Layout>
  );
